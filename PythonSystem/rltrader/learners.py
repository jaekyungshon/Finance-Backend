...
class ReinforcementLearner:
    ...
    def __init__(self, rl_method='rl', stock_code=None, 
                ...
                discount_factor=0.9, num_epoches=200,
    ...
    def init_value_network(self, shared_network=None, loss='mse'):
        ...
        if self.reuse_models and os.path.exists(self.value_network_path):
            #print(f"111111111 {self.value_network_path} 111111111111")
    ...
    def run(self, learning=True):
        ...
        # 에포크 반복
        #print(f"########### {self.num_epoches} ##############")
        print(f"######## {self.stock_code} Epoch start #########")
        idx=0
        for epoch in tqdm(range(self.num_epoches)):
            ...
            for i in tqdm(range(len(self.training_data)), leave=False): # 학습 데이터 개수
                # 샘플 생성
        ...
        try:
            ...
            print("### [Failed][learners.py][run] error create table ###")
            exit(0)
        #print(f"32094729034 this {self.memory_pv[-1]}")
        try:
            today=datetime.datetime.strptime(utils.get_today_str(),'%Y%m%d').date()
            sql=f"INSERT INTO aimodel VALUES ('{self.stock_code}','{self.memory_pv[-1]}','{today}') ON DUPLICATE KEY UPDATE code='{self.stock_code}', last_pv='{self.memory_pv[-1]}', date='{today}'"
            cur.execute(sql)
            conn.commit()
            print()
            print(f"### [Success][learners.py][run] Insert record '{self.stock_code}' in aimodel table ###")
            print()
        except:
            print()
            print("### [Failed][learners.py][run] Error Insert record in aimodel table ###")
            exit(0)
    ...
    def save_models(self):
        ...
        if self.policy_network is not None and self.policy_network_path is not None:
            print(f"222222222222 {self.policy_network_path} 222222222222222222")
    ...
    def predict(self):
        ...
        if self.gen_output:
            print(f"33333 {self.gen_output}")
            print(f"4444 {self.output_path}")
